...
// Export singleton instance — defer creation until app is ready
// In dev mode, electron-updater crashes if constructed before app.whenReady()
...
/**
 * Lazy proxy: defers all method calls to the real AppUpdater
 * which is only created when first accessed (after app.whenReady).
 */
export const appUpdater: AppUpdater = new Proxy({} as AppUpdater, {
  get(_target, prop, _receiver) {
    const instance = getAppUpdater();
