...
function ProviderCard({
  ...
  const handleFetchModels = async () => {
    ...
    setFetchedModels([]);
    try {
      if (!baseUrl.trim() || !(newKey.trim() || status?.hasKey)) {
        setFetchModelsError(t('aiProviders.dialog.fetchModelsNeedKey'));
        setFetchingModels(false);
        return;
      }
      const result = await hostApi.providers.fetchRemoteModels({
        baseUrl: baseUrl.trim(),
        apiKey: newKey.trim() || 'stored',
      });
      if (result.success && result.models.length > 0) {
        setFetchedModels(result.models);
      } else {
        setFetchModelsError(result.error || t('aiProviders.dialog.noModelsFound'));
      }
    } catch (error) {
              ...
              {account.vendorId === 'custom' && (
                <div className="space-y-1.5 pt-2">
                  ...
                  >
                    ...
                    {t('aiProviders.dialog.fetchModels', 'Fetch Models')}
                  </Button>
                  {fetchModelsError && (
                    <p className="text-xs text-red-500">{fetchModelsError}</p>
                  )}
                  {fetchedModels.length > 0 && (
                    <div className="space-y-1 max-h-32 overflow-y-auto bg-black/5 dark:bg-white/5 rounded-xl p-2">
                      {fetchedModels.map((m) => (
                        <button
                          key={m}
                          type="button"
                          className="w-full text-left px-3 py-1.5 text-sm rounded-lg hover:bg-black/10 dark:hover:bg-white/10 transition-colors"
                          onClick={() => {
                            setModelId(m);
                            setFetchedModels([]);
                            setFetchModelsError(null);
...
function AddProviderDialog({
  ...
  const handleFetchNewProviderModels = async () => {
    setFetchingModels(true);
    setFetchModelsError(null);
    setFetchedModels([]);
    try {
      if (!baseUrl.trim() || !apiKey.trim()) {
        setFetchModelsError('Please enter Base URL and API Key first');
        setFetchingModels(false);
        return;
      }
      const result = await hostApi.providers.fetchRemoteModels({
      ...
      if (result.success && result.models.length > 0) {
        ...
        setFetchModelsError(result.error || 'No models found');
      }
    } catch (error) {
      setFetchModelsError(error instanceof Error ? error.message : String(error));
    } finally {
                ...
                {selectedType === 'custom' && (
                  <div className="space-y-1.5 pt-2">
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      onClick={handleFetchNewProviderModels}
                      disabled={fetchingModels}
                      className="rounded-full h-8 text-xs"
                    >
                      {fetchingModels ? (
