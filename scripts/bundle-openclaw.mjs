...
function trimBundledOpenClawSkills(skillsRoot) {
  if (!fs.existsSync(skillsRoot))
    return { removed: 0, kept: [...BUNDLED_OPENCLAW_SKILL_ALLOWLIST] };
...
function removeDirRobust(targetDir) {
  ...
  try {
    ...
    if (
      ...
      (code === 'EACCES' || code === 'ENOTEMPTY' || code === 'EPERM')
    ) {
      try {
...
if (fs.existsSync(OUTPUT)) {
  echo`   Using existing build/openclaw directory...`;
} else {
...
for (const pkgName of EXTRA_BUNDLED_PACKAGES) {
  ...
  let pkgReal;
  try {
    pkgReal = fs.realpathSync(pkgLink);
  } catch {
    continue;
  }
...
const collectedEntries = [...collected].sort(
  ([leftRealPath, leftName], [rightRealPath, rightName]) => {
        ...
        if (pkgEntry.name.startsWith('@')) {
          ...
          for (const scopeEntry of scopeEntries) {
            ...
            try {
              ...
              fs.cpSync(normWin(srcScoped), normWin(destScoped), {
                recursive: true,
                dereference: true,
              });
